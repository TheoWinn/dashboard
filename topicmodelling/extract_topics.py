...
import numpy as np
from bert_utils import OnlineRepresentativeTracker, CachedEmbeddingBackend, clean_encoding_artifacts, extract_date_from_filename, RiverBERTopicWrapper, sliding_window


### SETUP ###

talkshow_path = "../youtube/data/clustered/talkshow_clustered/*.csv"
bundestag_path = "../youtube/data/clustered/bundestag_clustered/*.csv"#../matching/data/matched/*.csv" #"#<-yt transcript #"../matching/data/matched/*.csv" <- xml
output_path = "data/raw_topics"

#Pro Speaker doc
# Chunk Sinn? ->Yes
# Labels gut?
#topics_ info für bert labels
#merge topic model tryout
# topic model iterativ
...
speech_fillers = [
    ...
    #Talkshow spezifisch
    "Folge","Sendung", "Das war's für heute", "Hey","hey","bedanke","bedanken","Zuschauen",
]
...
embedding_model = SentenceTransformer("paraphrase-multilingual-mpnet-base-v2").to("cuda")#SentenceTransformer("intfloat/multilingual-e5-large-instruct").to("cuda")
cached_model = CachedEmbeddingBackend(embedding_model)
river_model = cluster.DBSTREAM(
    clustering_threshold=.75, # niedriger= strenger = weniger cluster
    minimum_weight=5.0, 
    intersection_factor=0.4, # Ab wann cluster gemerged werden  
    fading_factor=0.00001
)

dim_reduction_model = IncrementalPCA(n_components=10)
...
for i in range(0, total_docs, chunk_size):
    ...
    # 1. Calculate embeddings ONCE
    chunk_embeddings = embedding_model.encode(chunk, show_progress_bar=False)
    
    # 2. Store in cache wrapper
    cached_model.current_embeddings = chunk_embeddings
    
    # 3. Partial Fit (This will ask cached_model for embeddings and get them instantly)
    topic_model.partial_fit(chunk)
    
    # 4. Transform (Uses the specific embeddings we just calculated)
...
output_file = os.path.join(output_path, "topics_representations_2025_youtube.csv")
os.makedirs(output_path, exist_ok = True)
combined_df.to_csv(output_file, index = False)

topic_info.to_csv((os.path.join(output_path, "topic_info_2025_youtube.csv")), index = False)
