...
def main():
    ...
    parser.add_argument("--skip-bert_model",action="store_true", help= "Skip training the bert model")
    ...
    # Bert
    if not args.skip_match:
        cmd = [sys.executable, "add_new_topics.py"]
        if not run_step("add_new_topics.py", cmd, cwd=os.path.join(os.getcwd(), "topicmodelling", "src")):
            sys.exit(1)
