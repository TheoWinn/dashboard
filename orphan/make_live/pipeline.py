...
def main():
    parser = argparse.ArgumentParser(
        description="Full Processing Pipeline: Download -> Transcribe -> Cluster -> Match -> (BERT weekly) -> DB"
    )
    ...
    parser.add_argument(
        "--many-videos",
        action="store_true",
        help="Enable many videos mode (longer pauses, go through entire playlist).",
    )
    ...
    if not args.skip_transcribe:
        ...
        if not run_step("Transcribe Talkshows", cmd_ts, cwd=os.path.join(project_root, "youtube", "src")):
            sys.exit(1)

        # 3b. Bundestag
        cmd_bt = [sys.executable, "transcribe_audio.py", "--bundestag"]
        if not run_step("Transcribe Bundestag", cmd_bt, cwd=os.path.join(project_root, "youtube", "src")):
            sys.exit(1)
    ...
    if not args.skip_cluster:
        ...
        if not run_step("Cluster Talkshow Transcripts", cmd_ts, cwd=os.path.join(project_root, "youtube", "src")):
            sys.exit(1)

        # 4b. Bundestag
        cmd_bt = [sys.executable, "clean_aligned_transcripts.py", "--bundestag"]
        if not run_step("Cluster Bundestag Transcripts", cmd_bt, cwd=os.path.join(project_root, "youtube", "src")):
            sys.exit(1)

    # 5. Match Transcripts to Protocols
    if not args.skip_match:
        cmd = [sys.executable, "ma_utils.py"]
        if not run_step("Matching Pipeline", cmd, cwd=os.path.join(project_root, "matching", "src")):
            sys.exit(1)

    # 6. BERT (weekly) — guarded by flag
    if not args.skip_bert:
        # include Theos script

    # 7. Write to DB 
        # include Leonies script
